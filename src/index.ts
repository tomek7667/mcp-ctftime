...
async function getJson<T>(url: string): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`CTFtime API error ${res.status} for ${url}${text ? `: ${text.slice(0, 300)}` : ""}`);
  }
...
server.registerTool(
  "ctftime_events",
  {
    ...
    inputSchema: {
      limit: z.number().int().min(1).max(100).default(20).describe("Max events (1-100)."),
      start: z.number().int().optional().describe("UNIX timestamp (seconds) for window start."),
      finish: z.number().int().optional().describe("UNIX timestamp (seconds) for window finish."),
    },
  },
...
server.registerTool(
  "ctftime_top_teams",
  {
    ...
    inputSchema: {
      year: z.number().int().min(2011).max(2100).optional().describe("Year (e.g., 2025)."),
      limit: z.number().int().min(1).max(100).default(10).describe("Max teams (1-100)."),
    },
  },
  async ({ year, limit }) => {
    const base = year ? `${CTFtime_API_BASE}/top/${year}/` : `${CTFtime_API_BASE}/top/`;
...
server.registerTool(
  ...
  {
    description: "Get top teams for the current year by country (ISO 3166-1 alpha-2 country code, lowercase).",
    inputSchema: {
      country_code: z
        .string()
        .min(2)
...
server.registerTool(
  ...
  async ({ year }) => {
    const url = year ? `${CTFtime_API_BASE}/results/${year}/` : `${CTFtime_API_BASE}/results/`;
    const data = await getJson<any>(url);
    return {
      content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
    };
